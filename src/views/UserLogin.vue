<template>
  ...
  const USER_CREDENTIALS = ref({
    name: 'mouard',
    email: 'user.mourad@example.com',
    password: 'user123'
  });
  provide("USER_CREDENTIALS", USER_CREDENTIALS);
  
  const login = () => {
    isLoading.value = true;
    error.value = '';
  
    if (email.value === USER_CREDENTIALS.value.email && password.value === USER_CREDENTIALS.value.password) {
      localStorage.setItem('isUserLoggedIn', 'true'); // Store login status
      router.push('/UserMain'); // Redirect to UserView
    } else {
